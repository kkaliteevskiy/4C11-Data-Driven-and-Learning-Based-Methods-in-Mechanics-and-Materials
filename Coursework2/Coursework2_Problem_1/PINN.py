...
def objective(trial):
    ...
    for epoch in range(100): # NOTE: modify for debugging
        ...
        e_12 = dvdx[:,0].unsqueeze(1) + dudx[:,1].unsqueeze(1) #  maybe there is a factor of 0.5  
        ...
        e_12_bc = dudx_bc[:,1].unsqueeze(1) + dvdx_bc[:,0].unsqueeze(1) # * 0.5
...
study.optimize(objective, n_trials=100) # NOTE: modify for debugging
...
for epoch in range(iterations):
    ...
    e_11 = dudx[:,0].unsqueeze(1)
    e_22 = dvdx[:,1].unsqueeze(1)
    e_12 = (dvdx[:,0] + dudx[:,1]).unsqueeze(1)  # * 0.5
